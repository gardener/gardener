...
func GetMachineSpecFromCloudProfile(profile *gardencorev1beta1.CloudProfile) (vm MachineSpec, err error) {
	...
	defaultArchitecture := v1beta1helper.GetDefaultArchitectureFromCapabilityDefinitions(profile.Spec.Capabilities)
	...
	vm.ImageBaseName, err = getImageName(bastionSpec, profile.Spec.MachineImages, vm.Architecture, defaultArchitecture)
	if err != nil {
		return MachineSpec{}, err
	}
	vm.ImageVersion, err = getImageVersion(bastionSpec, vm.ImageBaseName, vm.Architecture, profile.Spec.MachineImages, defaultArchitecture)
	return vm, err
}
...
func findSupportedArchitectures(images []gardencorev1beta1.MachineImage, machineImageName, machineImageVersion, defaultArchitecture string) []string {
	...
	for _, image := range images {
		...
		for _, version := range image.Versions {
			...
			if version.Classification != nil && v1beta1helper.CurrentLifecycleClassification(version.ExpirableVersion) == gardencorev1beta1.ClassificationSupported {
				architectures.Insert(v1beta1helper.GetArchitecturesFromImageVersion(version, defaultArchitecture)...)
			}
...
func getImageArchitectures(bastion *gardencorev1beta1.Bastion, images []gardencorev1beta1.MachineImage, defaultArchitecture string) []string {
	var (
	...
	return findSupportedArchitectures(images, machineImageName, machineImageVersion, defaultArchitecture)
}

// getImageName returns the image name for the bastion.
func getImageName(bastion *gardencorev1beta1.Bastion, images []gardencorev1beta1.MachineImage, bastionArchitecture, defaultArchitecture string) (string, error) {
	...
	for _, image := range images {
		for _, version := range image.Versions {
			...
			if !slices.Contains(v1beta1helper.GetArchitecturesFromImageVersion(version, defaultArchitecture), bastionArchitecture) {
				continue
			}
			return image.Name, nil
		}
...
func getImageVersion(bastion *gardencorev1beta1.Bastion, imageName, machineArch string, images []gardencorev1beta1.MachineImage, defaultArchitecture string) (string, error) {
	...
	for _, version := range image.Versions {
		...
		if !slices.Contains(v1beta1helper.GetArchitecturesFromImageVersion(version, defaultArchitecture), machineArch) {
			continue
		}
...
func findMostSuitableMachineType(profile *gardencorev1beta1.CloudProfile) (machineName string, machineArch string, err error) {
	...
	supportedArchs := getImageArchitectures(profile.Spec.Bastion, profile.Spec.MachineImages, defaultArchitecture)

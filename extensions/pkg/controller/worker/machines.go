...
func WorkerPoolHashV1(pool extensionsv1alpha1.WorkerPool, cluster *extensionscontroller.Cluster, additionalData ...string) (string, error) {
	...
	if status := cluster.Shoot.Status; status.Credentials != nil && status.Credentials.Rotation != nil {
		if status.Credentials.Rotation.CertificateAuthorities != nil {
			if lastInitiationTime := v1beta1helper.LastInitiationTimeCARotationForWorkerPool(pool.Name, status.PendingWorkersUpdates, status.Credentials.Rotation.CertificateAuthorities.LastInitiationTime); lastInitiationTime != nil {
				data = append(data, lastInitiationTime.Time.String())
			}
		}
		if status.Credentials.Rotation.ServiceAccountKey != nil {
			if lastInitiationTime := v1beta1helper.LastInitiationTimeServiceAccountKeyRotationForWorkerPool(pool.Name, status.PendingWorkersUpdates, status.Credentials.Rotation.ServiceAccountKey.LastInitiationTime); lastInitiationTime != nil {
				data = append(data, lastInitiationTime.Time.String())
			}
		}
	}

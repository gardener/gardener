...
func add(mgr manager.Manager, args AddArgs, predicates []predicate.Predicate) error {
	...
	if args.IgnoreOperationAnnotation {
		if err := ctrl.Watch(
			...
			mapper.EnqueueRequestsFrom(ClusterToWorkerMapper(predicates), mapper.UpdateWithNew, mgr.GetLogger().WithName(ControllerName)),
		); err != nil {
			return err
		}
	}
...
func addStateUpdatingController(mgr manager.Manager, options controller.Options, extensionType string) error {
	...
	if err := ctrl.Watch(
		...
		mapper.EnqueueRequestsFrom(MachineSetToWorkerMapper(workerPredicates), mapper.UpdateWithNew, mgr.GetLogger().WithName(ControllerName)),
		machinePredicates...,
	); err != nil {
		return err
	}
	...
	return ctrl.Watch(
		...
		mapper.EnqueueRequestsFrom(MachineToWorkerMapper(workerPredicates), mapper.UpdateWithNew, mgr.GetLogger().WithName(ControllerName)),
		machinePredicates...,
	)
}

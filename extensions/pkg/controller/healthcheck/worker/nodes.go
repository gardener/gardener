	...
	for _, deployment := range machineDeploymentList.Items {
		for _, failedMachine := range deployment.Status.FailedMachines {
			...
			return &healthcheck.SingleCheckResult{
				...
				Detail: err.Error(),
				Codes:  gardencorev1beta1helper.DeprecatedDetermineErrorCodes(err), // TODO(@acumino): Drop error detection once extensions get adapted to parse error code from `SingleCheckResult`.
			}, nil
		}
	...
	if checkScaleUp {
		if status, err := checkNodesScalingUp(machineList, readyNodes, desiredMachines); status != gardencorev1beta1.ConditionTrue {
			...
			return &healthcheck.SingleCheckResult{
				Status:               status,
				Detail:               err.Error(),
				Codes:                []gardencorev1beta1.ErrorCode{}, // User error will not occure here.

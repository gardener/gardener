...
func (t *terraformer) execute(ctx context.Context, command string) error {
	...
	if pod != nil {
		...
		if status != podStatusSucceeded {
			...
			return gardencorev1beta1helper.DeprecatedDetermineError(errors.New(errorMessage)) // TODO(@acumino): Drop error detection once extension gets adapted to parse error code.
		}
	}

...
func (v *ValidateManagedSeed) admitGardenlet(ctx context.Context, gardenlet *seedmanagement.Gardenlet, shoot *gardencore.Shoot, fldPath *field.Path) (field.ErrorList, error) {
	...
	if gardenlet.Config != nil {
		...
		gardenletConfig, err := helper.DecodeGardenletConfigExternal(gardenlet.Config, false)
		if err != nil {
		...
		gardenletConfigJSON, err := json.Marshal(gardenletConfig)
		if err != nil {
			return allErrs, apierrors.NewInternalError(fmt.Errorf("could not marshal gardenlet config to JSON: %v", err))
		}
		gardenlet.Config = &runtime.RawExtension{Raw: gardenletConfigJSON}
	}
...
func (v *ValidateManagedSeed) admitSeedSpec(ctx context.Context, spec *gardencore.SeedSpec, shoot *gardencore.Shoot, fldPath *field.Path) (field.ErrorList, error) {
	...
	if spec.Ingress != nil {
		...
		if spec.DNS.IngressDomain == nil || *spec.DNS.IngressDomain == "" {
			...
			allErrs = append(allErrs, field.Invalid(fldPath.Child("dns", "ingressDomain"), spec.Ingress.Domain, fmt.Sprintf("seed ingress domain must be equal to shoot DNS domain %s", ingressDomain)))
		}
	}

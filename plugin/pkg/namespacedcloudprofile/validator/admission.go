...
func ValidateSimulatedNamespacedCloudProfileStatus(originalParentCloudProfile *gardencorev1beta1.CloudProfile, originalNamespacedCloudProfile *gardencorev1beta1.NamespacedCloudProfile) field.ErrorList {
	...
	namespacedcloudprofile.MergeCloudProfiles(parentCloudProfile, namespacedCloudProfile)

	coreCloudProfile := &gardencore.CloudProfile{}
	if err := api.Scheme.Convert(parentCloudProfile, coreCloudProfile, nil); err != nil {
		return field.ErrorList{{
			Type:     field.ErrorTypeInternal,
			Field:    "",
			BadValue: nil,
			Detail:   "could not convert parent CloudProfile from type core.gardener.cloud/v1beta1 to the internal core type",
		}}
	}
	return validation.ValidateNamespacedCloudProfileStatus(&coreCloudProfile.Spec, field.NewPath("status.cloudProfileSpec"))
}

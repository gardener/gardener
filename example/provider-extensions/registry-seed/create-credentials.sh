...
usage() {
  echo "Usage:"
  echo "> create_credentials.sh [ -h | <kubeconfig> <registry> ]"
  echo
  echo ">> For example: create_credentials.sh ~/.kube/kubeconfig.yaml registry.gardener.cloud"
...
htpasswd -Bbn gardener $password > $SCRIPT_DIR/auth

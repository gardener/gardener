...
// SameRegionTest makes sure that a shoot cannot be created in a different region than itself - given the scheduler is configured with SameRegion Strategy
func SameRegionTest(ctx context.Context, schedulerGardenerTest *framework.SchedulerGardenerTest) {
	...
	testLogger.Infof("Create shoot %s in namespace %s with unsupported Region %s", shootName, projectNamespace, unsupportedRegion)
	...
	if err != nil {
		testLogger.Fatalf("Failed to wait for shoot to be unschedulable: %s: %s", shootName, err.Error())
	}
}

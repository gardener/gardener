...
func (c *defaultControl) reconcileShoot(o *operation.Operation, operationType gardenv1beta1.ShootLastOperationType, updater UpdaterInterface) *gardenv1beta1.LastError {
	...
	var (
		...
		deployKubeAPIServer                  = f.AddTask(helperBotanist.DeployKubeAPIServer, defaultRetry, deploySecrets, deployETCD, waitUntilKubeAPIServerServiceIsReady, deployCloudProviderConfig, ensureImagePullSecretsSeed)
		...
		_                                    = f.AddTask(shootCloudBotanist.DeployAutoNodeRepair, defaultRetry, waitUntilKubeAPIServerIsReady, deployInfrastructure, ensureImagePullSecretsSeed)

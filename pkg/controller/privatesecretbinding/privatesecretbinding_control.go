...
func (c *Controller) reconcilePrivateSecretBindingKey(key string) error {
	...
	if err != nil {
		c.privateSecretBindingQueue.AddAfter(key, 15*time.Second)
	}
	return nil
}
...
func (c *defaultControl) ReconcilePrivateSecretBinding(obj *gardenv1beta1.PrivateSecretBinding, key string) error {
	...
	var (
		...
		privateSecretBindingJSON, _ = json.Marshal(privateSecretBinding)
	...
	privateSecretBindingLogger.Infof("[PRIVATESECRETBINDING RECONCILE] %s", key)
	privateSecretBindingLogger.Debugf(string(privateSecretBindingJSON))
	return nil
}

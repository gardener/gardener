...
func (b *Botanist) DefaultEtcd(role string, class etcd.Class) (etcd.Interface, error) {
	...
	e := NewEtcd(
		...
		b.Shoot.GetFailureToleranceType(),
		replicas,
...
func (b *Botanist) DeployEtcd(ctx context.Context) error {
	if b.Seed.GetInfo().Spec.Backup != nil {
		...
		if gardencorev1beta1helper.SeedSettingOwnerChecksEnabled(b.Seed.GetInfo().Spec.Settings) &&
			getEtdcReplicas(b.Shoot.GetInfo()) == 1 {
...
func (b *Botanist) ScaleUpETCD(ctx context.Context) error {
	return b.scaleETCD(ctx, getEtdcReplicas(b.Shoot.GetInfo()))
}
...
func getEtdcReplicas(shoot *gardencorev1beta1.Shoot) int32 {
	if gardenletfeatures.FeatureGate.Enabled(features.HAControlPlanes) && (metav1.HasAnnotation(shoot.ObjectMeta, v1beta1constants.ShootAlphaControlPlaneHighAvailability) || shoot.Spec.ControlPlane != nil) {
		return 3
	}
	return 1
}

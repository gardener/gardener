...
`
		...
		Context("k8s < 1.19", func() {
			It("should successfully deploy all resources", func() {
				names := append(defaultKCMControllerSANames, "default")
		...
		Context("1.19 <= k8s < 1.20", func() {
			BeforeEach(func() {
				values.KubernetesVersion = semver.MustParse("1.19.4")
				component = New(c, namespace, values)
			})

			It("should successfully deploy all resources", func() {
				names := append(defaultKCMControllerSANames, "default", "endpointslicemirroring-controller", "ephemeral-volume-controller")

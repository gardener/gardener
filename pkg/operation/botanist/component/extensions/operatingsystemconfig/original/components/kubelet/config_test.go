...
var _ = Describe("Config", func() {
	...
	DescribeTable("#Config",
		...
		Entry(
			"kubernetes 1.10 w/o defaults",
			"1.10.0",
			clusterDNSAddress,
			clusterDomain,
			components.ConfigurableKubeletConfigParameters{},
			kubeletConfigWithDefaults,
			func(cfg *kubeletconfigv1beta1.KubeletConfiguration) { cfg.RotateCertificates = false },
		),
		Entry(
			"kubernetes 1.10 w/ defaults",
			"1.10.0",
			clusterDNSAddress,
			clusterDomain,
			params,

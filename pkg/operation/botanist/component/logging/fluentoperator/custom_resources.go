...
function add_tag_to_record(tag, timestamp, record)
  ...
  return 1, timestamp, record
	...
	fluentBit := customresources.GetFluentBit(getFluentBitLabels(), fbName, c.namespace, c.values.FluentBitImage, c.values.FluentBitInitImage, c.values.FluentBitPriorityClass)
	resources = append(resources, fluentBit)

	clusterFluentBitConfig := customresources.GetClusterFluentBitConfig(fbName, getCustomResourcesLabels())

...
func (b *Botanist) DefaultCoreDNS() (coredns.Interface, error) {
	image, err := b.ImageVector.FindImage(imagevector.ImageNameCoredns, imagevectorutils.RuntimeVersion(b.ShootVersion()), imagevectorutils.TargetVersion(b.ShootVersion()))
	if err != nil {
		return nil, err
	}
	...
	if v1beta1helper.IsCoreDNSAutoscalingModeUsed(b.Shoot.GetInfo().Spec.SystemComponents, gardencorev1beta1.CoreDNSAutoscalingModeClusterProportional) {
		image, err = b.ImageVector.FindImage(imagevector.ImageNameClusterProportionalAutoscaler, imagevectorutils.RuntimeVersion(b.ShootVersion()), imagevectorutils.TargetVersion(b.ShootVersion()))
		if err != nil {
			return nil, err
		}

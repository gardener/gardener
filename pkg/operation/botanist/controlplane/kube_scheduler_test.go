...
var _ = Describe("KubeScheduler", func() {
	...
	BeforeEach(func() {
		...
		kubeScheduler, err = NewKubeScheduler(c, namespace, version, image, replicas, configEmpty)
	...
	Describe("#Deploy", func() {
		Context("missing secret information", func() {
			...
			It("should return an error because the kubeconfig secret information is not provided", func() {
				kubeScheduler.SetSecrets(KubeSchedulerSecrets{Kubeconfig: Secret{Name: secretNameKubeconfig, Checksum: secretChecksumKubeconfig}})
		...
		Context("secret information available", func() {
			...
			DescribeTable("success tests for various kubernetes versions",
				func(version string, config *gardencorev1beta1.KubeSchedulerConfig) {
					kubeScheduler, err = NewKubeScheduler(c, namespace, version, image, replicas, config)
					Expect(err).NotTo(HaveOccurred())
					...
					gomock.InOrder(
						c.EXPECT().Get(ctx, kutil.Key(namespace, configMapName), gomock.AssignableToTypeOf(&corev1.ConfigMap{})),
						c.EXPECT().Update(ctx, configMapFor(version)),
						c.EXPECT().Get(ctx, kutil.Key(namespace, vpaName), gomock.AssignableToTypeOf(&autoscalingv1beta2.VerticalPodAutoscaler{})),
						c.EXPECT().Update(ctx, vpa),
						c.EXPECT().Get(ctx, kutil.Key(namespace, serviceName), gomock.AssignableToTypeOf(&corev1.Service{})),
						c.EXPECT().Update(ctx, serviceFor(version)),
						c.EXPECT().Get(ctx, kutil.Key(namespace, deploymentName), gomock.AssignableToTypeOf(&appsv1.Deployment{})),
						c.EXPECT().Update(ctx, deploymentFor(version, config)),
					)
...
func componentConfigYAMLForKubernetesVersion(version string) (string, string) {
	apiVersion, checksum := "componentconfig/v1alpha1", "3370189444db371972ebc8add5b7c76944c9f2912735ed59b7660427b52373c6"

	if k8sVersionGreaterEqual118, _ := versionutils.CompareVersions(version, ">=", "1.18"); k8sVersionGreaterEqual118 {
		apiVersion, checksum = "kubescheduler.config.k8s.io/v1alpha2", "54b447618b8ce997b6e0d537be011ce18ff5046172980bf5efb7df7ceb6e97a9"
	} else if k8sVersionGreaterEqual112, _ := versionutils.CompareVersions(version, ">=", "1.12"); k8sVersionGreaterEqual112 {
		apiVersion, checksum = "kubescheduler.config.k8s.io/v1alpha1", "5995cdb0cc43de90d58325b7f11962c5b5f5d284bfbd3ee8736e0e97be8fbb0f"
	}

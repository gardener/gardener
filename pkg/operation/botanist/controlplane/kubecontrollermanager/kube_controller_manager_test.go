									...
									Containers: []corev1.Container{
										{
											...
											Command:         commandForKubernetesVersion(version, portForKubernetesVersion(version), config.NodeCIDRMaskSize, namespace, serviceCIDR, podCIDR, getHorizontalPodAutoscalerConfig(config.HorizontalPodAutoscalerConfig), kutil.FeatureGatesToCommandLineParameter(config.FeatureGates)),
...
func commandForKubernetesVersion(version string, port int32, nodeCIDRMaskSize *int32, clusterName string, serviceNetwork, podNetwork *net.IPNet, horizontalPodAutoscalerConfig *gardencorev1beta1.HorizontalPodAutoscalerConfig, featureGateFlags string) []string {
	var command []string

...
var _ = Describe("NamespacedCloudProfile Strategy", func() {
	...
	Describe("#WarningsOnUpdate", func() {
		...
		Describe("spec.limits.maxNodesTotal", func() {
			It("should not emit a warning if no limits are set", func() {
				Expect(namespacedcloudprofileregistry.Strategy.WarningsOnUpdate(ctx, namespacedCloudProfile, oldNamespacedCloudProfile)).To(BeEmpty())
			})

			It("should not emit a warning if a limit is newly added", func() {
				oldNamespacedCloudProfile.Spec.Limits = &core.Limits{MaxNodesTotal: nil}
				namespacedCloudProfile.Spec.Limits = &core.Limits{MaxNodesTotal: ptr.To[int32](10)}

				Expect(namespacedcloudprofileregistry.Strategy.WarningsOnUpdate(ctx, namespacedCloudProfile, oldNamespacedCloudProfile)).To(BeEmpty())
			})

			It("should not emit a warning if a limit is removed", func() {
				oldNamespacedCloudProfile.Spec.Limits = &core.Limits{MaxNodesTotal: ptr.To[int32](10)}
				namespacedCloudProfile.Spec.Limits = &core.Limits{MaxNodesTotal: nil}

				Expect(namespacedcloudprofileregistry.Strategy.WarningsOnUpdate(ctx, namespacedCloudProfile, oldNamespacedCloudProfile)).To(BeEmpty())
			})

			It("should not emit a warning if a limit remains unchanged", func() {
				oldNamespacedCloudProfile.Spec.Limits = &core.Limits{MaxNodesTotal: ptr.To[int32](10)}
				namespacedCloudProfile.Spec.Limits = &core.Limits{MaxNodesTotal: ptr.To[int32](10)}
			...
			It("should not emit a warning if a limit is increased", func() {
				oldNamespacedCloudProfile.Spec.Limits = &core.Limits{MaxNodesTotal: ptr.To[int32](10)}
				namespacedCloudProfile.Spec.Limits = &core.Limits{MaxNodesTotal: ptr.To[int32](11)}

				Expect(namespacedcloudprofileregistry.Strategy.WarningsOnUpdate(ctx, namespacedCloudProfile, oldNamespacedCloudProfile)).To(BeEmpty())
			})

			It("should emit a warning if a limit is decreased", func() {
				oldNamespacedCloudProfile.Spec.Limits = &core.Limits{MaxNodesTotal: ptr.To[int32](10)}
				namespacedCloudProfile.Spec.Limits = &core.Limits{MaxNodesTotal: ptr.To[int32](9)}

				Expect(namespacedcloudprofileregistry.Strategy.WarningsOnUpdate(ctx, namespacedCloudProfile, oldNamespacedCloudProfile)).To(ConsistOf(
					ContainSubstring("Reducing `spec.limits.maxNodesTotal` may lead to unintended consequences"),
				))
			})
		})
	})

...
var _ = Describe("Strategy", func() {
	...
	Describe("Defaulting Architecture", func() {
				...
				MachineImages: []core.MachineImage{
					{
						Name: "machineImage1",
						Versions: []core.MachineImageVersion{
							{ExpirableVersion: core.ExpirableVersion{Version: "2.1.0"}},
							{ExpirableVersion: core.ExpirableVersion{Version: "2.0.3"}},
						},
					},
					{
						Name: "machineImage2",
						Versions: []core.MachineImageVersion{
							{ExpirableVersion: core.ExpirableVersion{Version: "4.3.0"}},
							{ExpirableVersion: core.ExpirableVersion{Version: "4.2.3"}},
						},
					},
				},
			},
		...
		Describe("PrepareForCreate", func() {
			Context("when the cloudProfile HAS NO capabilitiesDefinition on Create", func() {
				It("should default the architecture for MachineImageVersion and MachineType on Create", func() {
					cloudProfile := cloudProfileBase.DeepCopy()
					cloudprofileregistry.Strategy.PrepareForCreate(context.TODO(), cloudProfile)

					verifyDefaultArchitectures(cloudProfile)
				})
			})
		...
		Describe("PrepareForUpdate", func() {
			Context("when the cloudProfile HAS NO capabilitiesDefinition on Update", func() {
				It("should default the architecture for MachineImageVersion and MachineType on Update", func() {
					cloudProfile := cloudProfileBase.DeepCopy()
...
func verifyEmptyArchitectures(cloudProfile *core.CloudProfile) {
	images := cloudProfile.Spec.MachineImages
	Expect(images[0].Versions[0].Architectures).To(BeEmpty())
	Expect(images[0].Versions[1].Architectures).To(BeEmpty())
	Expect(images[1].Versions[0].Architectures).To(BeEmpty())
	Expect(images[1].Versions[1].Architectures).To(BeEmpty())

	types := cloudProfile.Spec.MachineTypes
	Expect(types[0].Architecture).To(BeNil())
	Expect(types[1].Architecture).To(BeNil())
}

func verifyDefaultArchitectures(cloudProfile *core.CloudProfile) {
	images := cloudProfile.Spec.MachineImages
	Expect(images[0].Versions[0].Architectures).To(ConsistOf("amd64"))
	Expect(images[0].Versions[1].Architectures).To(ConsistOf("amd64"))
	Expect(images[1].Versions[0].Architectures).To(ConsistOf("amd64"))
	Expect(images[1].Versions[1].Architectures).To(ConsistOf("amd64"))

	types := cloudProfile.Spec.MachineTypes
	Expect(types[0].Architecture).To(PointTo(Equal("amd64")))
	Expect(types[1].Architecture).To(PointTo(Equal("amd64")))
}

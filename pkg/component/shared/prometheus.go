...
func NewPrometheus(log logr.Logger, c client.Client, namespace string, values prometheus.Values) (prometheus.Interface, error) {
	imagePrometheus, err := imagevector.Containers().FindImage(imagevector.ImageNamePrometheus)
	if err != nil {
		return nil, err
	}
	...
	{
		imageAlpine, err := imagevector.Containers().FindImage(imagevector.ImageNameAlpine)
		if err != nil {
			return nil, err
		}

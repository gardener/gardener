...
var _ = Describe("KubeAPIServer", func() {
	...
	Describe("#NewKubeAPIServer", func() {
		...
		BeforeEach(func() {
			...
			gardenClient = fakeclient.NewClientBuilder().WithScheme(kubernetesscheme.Scheme).Build()
		...
		Describe("AdmissionPlugins", func() {
			BeforeEach(func() {
				Expect(gardenClient.Create(ctx, secret)).To(BeNil())

...
func (o *otelCollector) Deploy(ctx context.Context) error {
	...
	if err := o.openTelemetryCollectorFeatureGateCleanup(ctx); err != nil {
		return fmt.Errorf("failed to delete valitail access secret during `OpenTelemetryCollector` featuregate activation: %w", err)
	}
...
func (o *otelCollector) openTelemetryCollectorFeatureGateCleanup(ctx context.Context) error {
	// Created only to access the `Secret` object.
	valitailAccessSecret := gardenerutils.NewShootAccessSecret("valitail", o.namespace).
	...
	return kubernetesutils.DeleteObjects(ctx, o.client, valitailAccessSecret.Secret)
}

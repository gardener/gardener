...
var _ = ginkgo.Describe("PrometheusRules", func() {
	ginkgo.Describe("#CentralPrometheusRules", func() {
		ginkgo.DescribeTable("return the expected objects",
			...
			ginkgo.Entry("workerless, w/o alertmanager", true, false, []string{"prometheus", "verticalpodautoscaler", "health", "kube-pods", "networking"}),
			ginkgo.Entry("workerless, w/ alertmanager", true, true, []string{"prometheus", "verticalpodautoscaler", "health", "kube-pods", "networking", "alertmanager"}),
			ginkgo.Entry("w/ workers, w/o alertmanager", false, false, []string{"prometheus", "verticalpodautoscaler", "health", "kube-kubelet", "kube-pods", "networking"}),
			ginkgo.Entry("w/ workers, w/ alertmanager", false, true, []string{"prometheus", "verticalpodautoscaler", "health", "kube-kubelet", "kube-pods", "networking", "alertmanager"}),
		)
		...
		ginkgo.It("should run the rules tests", func() {
			...
			test.PrometheusRule(workerHealth, "testdata/worker/health.prometheusrule.test.yaml")
			test.PrometheusRule(workerlessHealth, "testdata/workerless/health.prometheusrule.test.yaml")

...
var (
	...
	// worker rules
	//go:embed assets/prometheusrules/worker/health.yaml
	workerHealthYAML []byte
	workerHealth     *monitoringv1.PrometheusRule
	...
	// workerless rules
	//go:embed assets/prometheusrules/workerless/health.yaml
	workerlessHealthYAML []byte
	workerlessHealth     *monitoringv1.PrometheusRule
...
func init() {
	...
	// worker rules
	workerHealth = &monitoringv1.PrometheusRule{}
	utilruntime.Must(runtime.DecodeInto(monitoringutils.Decoder, workerHealthYAML, workerHealth))
	workerKubeKubelet = &monitoringv1.PrometheusRule{}
	...
	// workerless rules
	workerlessHealth = &monitoringv1.PrometheusRule{}
	utilruntime.Must(runtime.DecodeInto(monitoringutils.Decoder, workerlessHealthYAML, workerlessHealth))
	workerlessKubePods = &monitoringv1.PrometheusRule{}
...
func CentralPrometheusRules(isWorkerless, wantsAlertmanager bool) []*monitoringv1.PrometheusRule {
	...
	if isWorkerless {
		out = append(out, workerlessHealth.DeepCopy(), workerlessKubePods.DeepCopy(), workerlessNetworking.DeepCopy())
	} else {
		out = append(out, workerHealth.DeepCopy(), workerKubeKubelet.DeepCopy(), workerKubePods.DeepCopy(), workerNetworking.DeepCopy())
	}

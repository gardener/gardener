						...
						Rules: []monitoringv1.Rule{
							...
							},
							{
								...
								Annotations: map[string]string{
									...
									"description": "Etcd3 " + role + ` DB size has reached {{ with query (printf "(etcd_mvcc_db_total_size_in_bytes{pod=\"%s\"} / on (pod) group_left etcd_server_quota_backend_bytes{pod=\"%s\"}) * 100" $labels.pod $labels.pod) }}{{ . | first | value | printf "%.0f" }}{{ end }}% of its configured etcd quota. DB size: {{ with query (printf "etcd_mvcc_db_total_size_in_bytes{pod=\"%s\"}" $labels.pod) }}{{ . | first | value | humanize1024 }}B{{ end }}, quota: {{ with query (printf "etcd_server_quota_backend_bytes{pod=\"%s\"}" $labels.pod) }}{{ . | first | value | humanize1024 }}B{{ end }}. Etcd quota might need to be increased.`,
								},
							},
							{

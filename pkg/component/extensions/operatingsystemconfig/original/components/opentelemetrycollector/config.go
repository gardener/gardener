...
func getOpentelemetryCollectorConfigurationFile(ctx components.Context) (extensionsv1alpha1.File, error) {
	...
	if err := tplOpenTelemetryCollector.Execute(&config, map[string]any{
		"clientURL":       "https://" + ctx.OpenTelemetryCollectorIngressHostName + collectorconstants.PushEndpoint,
...
// ExecStartPre=/bin/sh -c "systemctl set-environment HOSTNAME=$(hostname | tr [:upper:] [:lower:]) KUBECONFIG=` + OpenTelemetryCollectorKubeconfigPath + `"

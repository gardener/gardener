...
var _ = Describe("ChartRenderer", func() {
	...
	Describe("#RenderEmbeddedFS", func() {
		...
		It("should return rendered chart", func() {
			...
			Expect(files).To(HaveKey("alpine/templates/alpine-charts.yaml"))
			Expect(files["alpine/templates/alpine-charts.yaml"]).To(HaveKeyWithValue("pod/alpine", alpinePod))
			Expect(files["alpine/templates/alpine-charts.yaml"]).To(HaveKeyWithValue("secret/test", testSecret))
	...
	Describe("#FileContent", func() {
		...
		It("should return the file content when template file exists", func() {
			...
			actual := chart.FileContent("alpine-charts.yaml")
	...
	Describe("#AsSecretData", func() {
		It("should return rendered chart as secret data", func() {
			...
			Expect(string(data["alpine_templates_alpine-charts_secret_test.yaml"])).To(Equal(testSecret))
			Expect(string(data["alpine_templates_alpine-charts_pod_alpine.yaml"])).To(Equal(alpinePod))
			Expect(string(data["alpine_templates_alpine-charts_clusterrole_gardener.cloud_test.yaml"])).To(Equal(testClusterRole))
		})
	})
})

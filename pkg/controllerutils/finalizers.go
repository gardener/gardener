...
// AddFinalizers ensures that the given finalizer is present in the given object with exponential backoff and optimistic
// locking. If it is not set, it adds it and issues a patch.
...
// RemoveFinalizers ensures that the given finalizer is not present anymore in the given object with exponential backoff
// and optimistic locking. If it is set, it removes it and issues a patch.
// Note that this is done with a regular merge-patch since strategic merge-patches do not work with custom resources,
// see https://github.com/kubernetes/kubernetes/issues/105146.
...
func RemoveAllFinalizers(ctx context.Context, writer client.Writer, obj client.Object) error {
	...
	return client.IgnoreNotFound(writer.Patch(ctx, obj, mergeFromWithOptimisticLock(beforePatch)))
}

...
// List lists all ControllerDeployments in the indexer.
func (s *controllerDeploymentLister) List(selector labels.Selector) (ret []*v1alpha1.ControllerDeployment, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.ControllerDeployment))
	})
	return ret, err
}

// Get retrieves the ControllerDeployment from the index for a given name.
func (s *controllerDeploymentLister) Get(name string) (*v1alpha1.ControllerDeployment, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("controllerdeployment"), name)
	}
	return obj.(*v1alpha1.ControllerDeployment), nil
}

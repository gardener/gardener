...
type ShootExtensionStatusLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.ShootExtensionStatus, err error)
...
// List lists all ShootExtensionStatuses in the indexer.
func (s *shootExtensionStatusLister) List(selector labels.Selector) (ret []*v1alpha1.ShootExtensionStatus, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.ShootExtensionStatus))
	})
	return ret, err
}
...
type ShootExtensionStatusNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.ShootExtensionStatus, error)
...
// List lists all ShootExtensionStatuses in the indexer for a given namespace.
func (s shootExtensionStatusNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.ShootExtensionStatus, err error) {
...
// Get retrieves the ShootExtensionStatus from the indexer for a given namespace and name.
func (s shootExtensionStatusNamespaceLister) Get(name string) (*v1alpha1.ShootExtensionStatus, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("shootextensionstatus"), name)
	}
	return obj.(*v1alpha1.ShootExtensionStatus), nil
}

...
func (c *Clientset) ListStatefulSets(namespace string, listOptions metav1.ListOptions) (*appsv1.StatefulSetList, error) {
	statefulsets, err := c.Kubernetes().AppsV1().StatefulSets(namespace).List(listOptions)
	if err != nil {
		return nil, err
	}
...
func (c *Clientset) DeleteStatefulSet(namespace, name string) error {
	return c.Kubernetes().AppsV1().StatefulSets(namespace).Delete(name, &defaultDeleteOptions)
}

...
type BastionLister interface {
	...
	List(selector labels.Selector) (ret []*operations.Bastion, err error)
...
// List lists all Bastions in the indexer.
func (s *bastionLister) List(selector labels.Selector) (ret []*operations.Bastion, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*operations.Bastion))
	})
	return ret, err
}
...
type BastionNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*operations.Bastion, error)
...
// List lists all Bastions in the indexer for a given namespace.
func (s bastionNamespaceLister) List(selector labels.Selector) (ret []*operations.Bastion, err error) {
...
// Get retrieves the Bastion from the indexer for a given namespace and name.
func (s bastionNamespaceLister) Get(name string) (*operations.Bastion, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(operations.Resource("bastion"), name)
	}
	return obj.(*operations.Bastion), nil
}

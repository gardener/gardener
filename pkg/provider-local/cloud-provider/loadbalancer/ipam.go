...
func (p *Provider) allocateLoadBalancerIPs(ctx context.Context, service *corev1.Service) (*loadBalancerIPs, error) {
	kindNetwork, err := p.DockerClient.NetworkInspect(ctx, defaultNetwork, network.InspectOptions{})
	if err != nil {
		return nil, fmt.Errorf("failed to inspect network %s: %w", defaultNetwork, err)
	}

	allocatedIPs := sets.New[netip.Addr]()
	for _, c := range kindNetwork.Containers {
		if c.IPv4Address != "" {

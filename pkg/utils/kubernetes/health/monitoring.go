...
func IsPrometheusHealthy(ctx context.Context, endpoint string, port int) PrometheusHealthCheckResult {
	...
	if len(warnings) > 0 {
		return PrometheusHealthCheckResult{Error: fmt.Errorf("query returned warnings: %s", strings.Join(warnings, ", "))}
	}

	vector, ok := result.(model.Vector)
	if !ok {
		return PrometheusHealthCheckResult{Error: fmt.Errorf("query returned an unexpected result type: %s", result.Type())}
	}

	if len(vector) == 0 {
		return PrometheusHealthCheckResult{IsHealthy: false, Message: "health check recording rules are not deployed or running yet"}
	}

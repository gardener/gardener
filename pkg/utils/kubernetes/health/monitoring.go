...
type PrometheusHealthChecker func(ctx context.Context, endpoint string, port int) PrometheusHealthCheckResult

// IsPrometheusHealthy checks for health issues in a Prometheus instance.
func IsPrometheusHealthy(ctx context.Context, endpoint string, port int) PrometheusHealthCheckResult {
	client, err := prom.NewClient(prom.Config{Address: fmt.Sprintf("http://%s:%d", endpoint, port)})
	if err != nil {
		return PrometheusHealthCheckResult{Error: fmt.Errorf("failed to create Prometheus client: %w", err)}
	}
	...
	if err != nil {
		return PrometheusHealthCheckResult{Error: fmt.Errorf("query failed: %w", err)}
	}

	if len(warnings) > 0 {
		return PrometheusHealthCheckResult{Error: fmt.Errorf("query returned warnings")}
	}

	if result.Type() != model.ValVector {
		return PrometheusHealthCheckResult{Error: fmt.Errorf("query returned an unexpected result type")}
	}

	vector := result.(model.Vector)
	if len(vector) == 0 {
		return PrometheusHealthCheckResult{Error: fmt.Errorf("recording rules are not deployed or running yet")}
	} else if len(vector) > 1 {
		return PrometheusHealthCheckResult{Error: fmt.Errorf("query returned multiple results but a single value was expected")}
	}

	isHealthy := vector[0].Value == 1
	return PrometheusHealthCheckResult{IsHealthy: isHealthy}
}

...
var _ = Describe("Monitoring", func() {
	...
	Describe("IsPrometheusHealthy", func() {
		...
		It("should handle prometheus error responses", func() {
			...
			Expect(result.Error.Error()).To(ContainSubstring("invalid query"))
			Expect(result.IsHealthy).To(BeFalse())
			Expect(result.Message).To(Equal(""))
		})
		...
		It("should handle vector response with unexpected samples", func() {
			...
			result := health.IsPrometheusHealthy(context.Background(), endpoint, port)
			Expect(result.Error).To(HaveOccurred())
			Expect(result.Error.Error()).To(Equal(`query returned inconsistent sample values: healthcheck:up{task="bar"} => 2, healthcheck:up{task="foo"} => 2`))
			Expect(result.IsHealthy).To(BeFalse())
			Expect(result.Message).To(Equal(""))
		})
		...
		It("should handle vector response with multiple healthy samples", func() {
			...
			result := health.IsPrometheusHealthy(context.Background(), endpoint, port)
			Expect(result.Error).To(HaveOccurred())
			Expect(result.Error.Error()).To(Equal(`query returned inconsistent sample values: healthcheck:up => 1, healthcheck:up{task="foo"} => 1`))
			Expect(result.IsHealthy).To(BeFalse())
			Expect(result.Message).To(Equal(""))
		})
		...
		It("should handle vector response with healthy and unhealthy samples", func() {
			...
			result := health.IsPrometheusHealthy(context.Background(), endpoint, port)
			Expect(result.Error).To(HaveOccurred())
			Expect(result.Error.Error()).To(Equal(`query returned inconsistent sample values: healthcheck:up => 1, healthcheck:up{task="foo"} => 0`))
			Expect(result.IsHealthy).To(BeFalse())
			Expect(result.Message).To(Equal(""))
		})
		...
		It("should handle timeouts", func() {
			...
			result := health.IsPrometheusHealthy(ctx, endpoint, port)
			Expect(result.Error).To(HaveOccurred())
			Expect(result.Error.Error()).To(Equal(fmt.Sprintf(`query failed: Post "http://%s:%d/api/v1/query": context deadline exceeded`, endpoint, port)))
			Expect(result.IsHealthy).To(BeFalse())
		})
	})
})

...
// ReconcileTopologyAwareRoutingSettings adds or removes the required annotation and label to make a Service topology-aware.
//
...
func ReconcileTopologyAwareRoutingSettings(service *corev1.Service, topologyAwareRoutingEnabled bool, k8sVersion *semver.Version) {
	...
	if versionutils.ConstraintK8sGreaterEqual132.Check(k8sVersion) {
		// For Kubernetes >= 1.32, use onlythe PreferClose strategy of the ServiceTrafficDistribution feature.
	...
	} else if versionutils.ConstraintK8sGreaterEqual127.Check(k8sVersion) {
		// For Kubernetes >= 1.27 and < 1.31, use the TopologyAwareHints feature (with the new annotation key) in combination with the GRM's endpoints-slice-hints webhook.
		metav1.SetMetaDataAnnotation(&service.ObjectMeta, corev1.AnnotationTopologyMode, "auto")
		metav1.SetMetaDataLabel(&service.ObjectMeta, resourcesv1alpha1.EndpointSliceHintsConsider, "true")
	} else {
		// For Kubernetes < 1.27, use the TopologyAwareHints feature (with the deprecated annotation key) in combination with the GRM's endpoints-slice-hints webhook.

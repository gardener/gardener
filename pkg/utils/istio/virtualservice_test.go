...
var _ = Describe("VirtualService", func() {
	...
	DescribeTable("ConnectionUpgradeRegex",
		...
		Entry("no match for words separated by space instead of comma", "hkfsjdhfkjsdhfsd Upgrade fddfsdfsdf", false),
		Entry("no match for prefix hyphen", "no-upgrade, foo", false),
		Entry("no match for suffix hyphen", "foo, upgrade-something", false),

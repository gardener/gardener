...
// This logic should not change between kubernetes versions, only if the default docker registry changes which is very unlikely.
...
// GetImagePullSecretForImage returns the pull secret for the given image if the image has a PullSecretRef. Otherwise, it returns nil.
func GetImagePullSecretForImage(ctx context.Context, c client.Client, image *Image, namespace string) (*corev1.Secret, error) {
	if image.PullSecretRef == nil {
		return nil, nil
	}

	secret := &corev1.Secret{}
	if err := c.Get(ctx, client.ObjectKey{Namespace: namespace, Name: *image.PullSecretRef}, secret); err != nil {
		return nil, fmt.Errorf("failed to get pull secret %q in namespace %q: %w", *image.PullSecretRef, namespace, err)
	}

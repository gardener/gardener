...
func jitteryDuration(totalDuration float64, minPercentage, maxPercentage *int32) time.Duration {
	var (
		minPercentageValue = ptr.Deref(minPercentage, 70)
		maxPercentageValue = ptr.Deref(maxPercentage, 90)

		minFactor = 1 - float64(minPercentageValue)/100
		maxFactor = float64(maxPercentageValue-minPercentageValue) / 100
	)

...
func init() {
	...
	r.FS.MkdirAll("/etc/containerd", 0755)
...
var _ = Describe("containerd configuration file tests", func() {
	...
	Describe("static containerd configuration paths set by gardener-node-agent", func() {
		...
		When("containerd configuration file version is v2", func() {
			BeforeEach(func() {
				err := loadContainerdConfig("testfiles/containerd-config.toml-v2", r.FS)
			...
			It("should set the imports", func() {
				...
				Expect(ok).To(BeTrue())
				Expect(len(rawImports)).To(Equal(1))
			...
			It("should set the cgroup driver", func() {
				...
				Expect(ok).To(BeTrue())
				Expect(v).To(Equal(systemdCgroupValue))
			})
		...
		When("containerd configuration file version is v3", func() {
			BeforeEach(func() {
				err := loadContainerdConfig("testfiles/containerd-config.toml-v3", r.FS)
				Expect(err).ToNot(HaveOccurred())
	...
	Describe("plugin configuration paths inserted by osc plugin config", func() {
		...
		When("containerd configuration file version is v2", func() {
			...
			It("should not translate anything", func() {
				...
				translatedPath := structuredmap.Path{"plugins", "io.containerd.cri.v1.runtime", "containerd", "runtimes", "foo", "runtime_type"}
				_, err = getContainerdConfigValue(r.FS, translatedPath)
				Expect(err).To(HaveOccurred())

				path := structuredmap.Path{"plugins", "io.containerd.grpc.v1.cri", "containerd", "runtimes", "foo", "runtime_type"}
				configValue, err := getContainerdConfigValue(r.FS, path)
				Expect(err).ToNot(HaveOccurred())
		...
		When("containerd configuration file version is v3", func() {
			...
			It("should translate a v2 compliant runtime path to its v3 equivalent", func() {
				...
				untranslatedPath := structuredmap.Path{"plugins", "io.containerd.grpc.v1.cri", "containerd", "runtimes", "foo", "runtime_type"}
				_, err = getContainerdConfigValue(r.FS, untranslatedPath)
				Expect(err).To(HaveOccurred())

				path := structuredmap.Path{"plugins", "io.containerd.cri.v1.runtime", "containerd", "runtimes", "foo", "runtime_type"}
				configValue, err := getContainerdConfigValue(r.FS, path)
				Expect(err).ToNot(HaveOccurred())
			...
			It("should not translate a path that is not in the translation map", func() {
				...
				translatedPath := structuredmap.Path{"plugins", "io.containerd.cri.v1.runtime", "containerd", "foobar", "foo", "runtime_type"}
				_, err = getContainerdConfigValue(r.FS, translatedPath)
				Expect(err).To(HaveOccurred())

				path := structuredmap.Path{"plugins", "io.containerd.grpc.v1.cri", "containerd", "foobar", "foo", "runtime_type"}
				configValue, err := getContainerdConfigValue(r.FS, path)
				Expect(err).ToNot(HaveOccurred())

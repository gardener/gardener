...
func ValidateShootSpec(meta metav1.ObjectMeta, spec *core.ShootSpec, opts shootValidationOptions, fldPath *field.Path, inTemplate bool) field.ErrorList {
	...
	allErrs = append(allErrs, validateSelfHostedShootControlPlaneHA(spec, fldPath)...)
...
func ValidateWorker(worker core.Worker, kubernetes core.Kubernetes, shootNamespace, shootProviderType string, fldPath *field.Path, inTemplate bool) field.ErrorList {
	...
	if worker.ControlPlane != nil {
		if worker.Minimum != worker.Maximum {
			allErrs = append(allErrs, field.Invalid(fldPath.Child("minimum"), worker.Minimum, "minimum must be equal maximum for the control plane worker pool"))
		}
...
func validateSelfHostedShootControlPlaneHA(spec *core.ShootSpec, fldPath *field.Path) field.ErrorList {
	allErrs := field.ErrorList{}
	...
	workerPath := fldPath.Child("provider", "workers")
	for i, worker := range spec.Provider.Workers {
		if worker.ControlPlane != nil {
			workerPath = workerPath.Index(i)
			break
		}
	}

...
import (
	...
	extensionruntime "github.com/gardener/gardener/pkg/operator/controller/extension/runtime"
...
func (r *Reconciler) AddToManager(ctx context.Context, mgr manager.Manager, gardenClientMap clientmap.ClientMap) error {
	...
	r.registerExtensionResourceWatchesFunc = func() error {
		for _, extension := range extensionKinds {
			...
			if err := c.Watch(source.Kind(mgr.GetCache(), extension.object), eventHandler, extensions.ObjectPredicate()); err != nil {
				return err
			}

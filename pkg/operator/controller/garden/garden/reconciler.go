...
func (r *Reconciler) generateGlobalObservabilityIngressPassword(ctx context.Context, secretsManager secretsmanager.Interface) (*corev1.Secret, error) {
	...
	patch := client.MergeFrom(newSecret.DeepCopy())
	metav1.SetMetaDataLabel(&newSecret.ObjectMeta, v1beta1constants.GardenRole, v1beta1constants.GardenRoleGlobalMonitoring)
	if err := r.RuntimeClientSet.Client().Patch(ctx, newSecret, patch); err != nil {
		return nil, fmt.Errorf("failed to add role label to global observability secret in runtime cluster: %w", err)
	}

	// Clean up the old password if it was rotated.

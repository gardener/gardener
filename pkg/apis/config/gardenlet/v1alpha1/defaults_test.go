...
var _ = Describe("Defaults", func() {
	...
	Describe("GardenletConfiguration defaulting", func() {
		It("should default the gardenlet configuration", func() {
			...
			Expect(obj.SeedConfig).To(BeNil())
		...
		It("should not overwrite already set values for the gardenlet configuration", func() {
			obj.LogLevel = config.LogLevelDebug
			obj.LogFormat = config.LogFormatText
			SetObjectDefaults_GardenletConfiguration(obj)

			Expect(obj.LogLevel).To(Equal(config.LogLevelDebug))
			Expect(obj.LogFormat).To(Equal(config.LogFormatText))
		})
	})
	...
	Describe("SeedConfig defaulting", func() {
		It("should not default the SeedConfig if none was provided", func() {
			SetObjectDefaults_GardenletConfiguration(obj)
			Expect(obj.SeedConfig).To(BeNil())
		})

		It("should default SeedConfig.Spec identically to the Seed defaulting", func() {
			expectedSeed := &gardencorev1beta1.Seed{}
			gardencorev1beta1.SetObjectDefaults_Seed(expectedSeed)
			obj.SeedConfig = &SeedConfig{}
			SetObjectDefaults_GardenletConfiguration(obj)
			Expect(obj.SeedConfig.Spec).To(Equal(expectedSeed.Spec))
		})

		It("should not overwrite already set values for the SeedConfig", func() {

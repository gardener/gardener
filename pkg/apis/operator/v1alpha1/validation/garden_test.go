	...
	Describe("#ValidateGarden", func() {
		...
		Context("runtime cluster", func() {
			...
			Context("Ingress", func() {
				...
				It("should complain about invalid Garden prometheus replicas", func() {
					garden.Spec.RuntimeCluster.Monitoring = &operatorv1alpha1.Monitoring{
						PrometheusGarden: &operatorv1alpha1.PrometheusConfig{
							Replicas: -1,
						},
						PrometheusLongterm: &operatorv1alpha1.PrometheusConfig{
							Replicas: -1,
						},
					}
					...
					Expect(ValidateGarden(garden, extensions)).To(ConsistOf(
						PointTo(MatchFields(IgnoreExtras, Fields{
							"Type":  Equal(field.ErrorTypeInvalid),
							"Field": Equal("spec.runtimeCluster.monitoring.prometheusGarden.replicas"),
						})),
						PointTo(MatchFields(IgnoreExtras, Fields{
							"Type":  Equal(field.ErrorTypeInvalid),
							"Field": Equal("spec.runtimeCluster.monitoring.prometheusLongterm.replicas"),
						})),
					))
				})
				...
				It("should complain about invalid Garden prometheus retention", func() {
					...
					Expect(ValidateGarden(garden, extensions)).To(ConsistOf(
						PointTo(MatchFields(IgnoreExtras, Fields{
							"Type":  Equal(field.ErrorTypeInvalid),
							"Field": Equal("spec.runtimeCluster.monitoring.prometheusGarden.retention"),
						})),
						PointTo(MatchFields(IgnoreExtras, Fields{
							"Type":  Equal(field.ErrorTypeInvalid),
							"Field": Equal("spec.runtimeCluster.monitoring.prometheusLongterm.retention"),
						})),
					))
				})
				...
				It("should complain about invalid Garden prometheus storage size", func() {
					...
					Expect(ValidateGarden(garden, extensions)).To(ConsistOf(
						PointTo(MatchFields(IgnoreExtras, Fields{
							"Type":  Equal(field.ErrorTypeInvalid),
							"Field": Equal("spec.runtimeCluster.monitoring.prometheusGarden.storage.capacity"),
						})),
						PointTo(MatchFields(IgnoreExtras, Fields{
							"Type":  Equal(field.ErrorTypeInvalid),
							"Field": Equal("spec.runtimeCluster.monitoring.prometheusLongterm.storage.capacity"),
						})),
					))
				})
				...
				It("should complain if Garden prometheus className is invalid", func() {
					...
					Expect(ValidateGarden(garden, extensions)).To(ConsistOf(
						PointTo(MatchFields(IgnoreExtras, Fields{
							"Type":  Equal(field.ErrorTypeInvalid),
							"Field": Equal("spec.runtimeCluster.monitoring.prometheusGarden.storage.className"),
						})),
						PointTo(MatchFields(IgnoreExtras, Fields{
							"Type":  Equal(field.ErrorTypeInvalid),
							"Field": Equal("spec.runtimeCluster.monitoring.prometheusLongterm.storage.className"),
						})),
					))
				})
			})

...
type GardenerDashboardConfig struct {
	...
	// IncludeCAForSNI specifies whether to include the CA certificate from the SNI TLS secret
	// in the dashboard's apiServerCaData configuration (used for all kubeconfigs generated by the
	// dashboard, e.g. for terminal sessions, OIDC login, or service account access).
	// Enable this when the API server SNI endpoint is signed by a private/custom CA.
	// For publicly trusted CAs (e.g. Let's Encrypt), leave this disabled to allow transparent
	// CA rotation without requiring kubeconfig re-downloads.
	// This field is only relevant when .spec.virtualCluster.kubernetes.kubeAPIServer.sni is configured.
	// Defaults to false.
	// +optional
	IncludeCAForSNI *bool `json:"includeCAForSNI,omitempty"`
}

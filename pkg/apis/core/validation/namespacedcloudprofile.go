...
func ValidateNamespacedCloudProfile(namespacedCloudProfile *core.NamespacedCloudProfile) field.ErrorList {
	...
	if namespacedCloudProfile.Spec.Kubernetes != nil {
		allErrs = append(allErrs, validateOnlyExpirationsKubernetesVersions(namespacedCloudProfile.Spec.Kubernetes.Versions, field.NewPath("spec.kubernetes"))...)
	}
	allErrs = append(allErrs, validateOnlyExpirationsMachineImages(namespacedCloudProfile.Spec.MachineImages, field.NewPath("spec.machineImages"))...)

	if len(namespacedCloudProfile.Spec.MachineTypes) > 0 {
	...
	if len(namespacedCloudProfile.Spec.Regions) != 0 {
		allErrs = append(allErrs, validateRegions(namespacedCloudProfile.Spec.Regions, field.NewPath("spec.regions"))...)
	}
...
func validateOnlyExpirationsKubernetesVersions(versions []core.ExpirableVersion, fldPath *field.Path) field.ErrorList {
	allErrs := field.ErrorList{}
	...
	allErrs = append(allErrs, validateSpecStaticKubernetesVersions(versions, fldPath)...)
	return allErrs
}

func validateOnlyExpirationsMachineImages(machineImages []core.MachineImage, fldPath *field.Path) field.ErrorList {
	allErrs := field.ErrorList{}

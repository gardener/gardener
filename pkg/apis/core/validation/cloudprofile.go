...
func validateMachineTypes(machineTypes []core.MachineType, fldPath *field.Path) field.ErrorList {
	...
	for i, machineType := range machineTypes {
		...
		allErrs = append(allErrs, corevalidation.ValidateResourceQuantityValue("cpu", machineType.CPU, cpuPath)...)
		allErrs = append(allErrs, corevalidation.ValidateResourceQuantityValue("gpu", machineType.GPU, gpuPath)...)
		allErrs = append(allErrs, corevalidation.ValidateResourceQuantityValue("memory", machineType.Memory, memoryPath)...)
...
func validateMachineTypeStorage(storage core.MachineTypeStorage, fldPath *field.Path) field.ErrorList {
	...
	if storage.StorageSize != nil {
		allErrs = append(allErrs, corevalidation.ValidateResourceQuantityValue("size", *storage.StorageSize, fldPath.Child("size"))...)
	}

	if storage.MinSize != nil {
		allErrs = append(allErrs, corevalidation.ValidateResourceQuantityValue("minSize", *storage.MinSize, fldPath.Child("minSize"))...)
	}
...
func validateVolumeTypes(volumeTypes []core.VolumeType, fldPath *field.Path) field.ErrorList {
	...
	for i, volumeType := range volumeTypes {
		...
		if volumeType.MinSize != nil {
			allErrs = append(allErrs, corevalidation.ValidateResourceQuantityValue("minSize", *volumeType.MinSize, idxPath.Child("minSize"))...)
		}
	}

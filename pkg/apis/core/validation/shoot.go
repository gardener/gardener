...
var (
	...
	availableShootOperationsToRunInParallel = sets.New(
		v1beta1constants.GardenerOperationReconcile,
		v1beta1constants.OperationRotateCredentialsStart,
		v1beta1constants.OperationRotateCredentialsStartWithoutWorkersRollout,
		v1beta1constants.OperationRotateCredentialsComplete,
...
func validateShootOperation(operations, maintenanceOperations []string, shoot *core.Shoot, fldPath *field.Path) field.ErrorList {
	...
	if !features.DefaultFeatureGate.Enabled(features.CredentialsRotationWithoutWorkersRollout) {
		...
		for _, op := range operations {
			if op != "" && (restrictedOperations.Has(op) || strings.HasPrefix(op, v1beta1constants.OperationRotateRolloutWorkers)) {
				allErrs = append(allErrs, field.Forbidden(fldPathOp, fmt.Sprintf("the %s operation can only be used when the CredentialsRotationWithoutWorkersRollout feature gate is enabled", op)))
			}
		}
		for _, mOp := range maintenanceOperations {
			if mOp != "" && (restrictedOperations.Has(mOp) || strings.HasPrefix(mOp, v1beta1constants.OperationRotateRolloutWorkers)) {
				allErrs = append(allErrs, field.Forbidden(fldPathMaintOp, fmt.Sprintf("the %s operation can only be used when the CredentialsRotationWithoutWorkersRollout feature gate is enabled", mOp)))
			}
		}
	}
	...
	for _, op := range operations {
		...
		} else if len(operations) > 1 && !availableShootOperationsToRunInParallel.Has(op) && !strings.HasPrefix(op, v1beta1constants.OperationRotateRolloutWorkers) {
		...
		if helper.IsShootInHibernation(shoot) &&
			(forbiddenShootOperationsWhenHibernated.Has(op) || strings.HasPrefix(op, v1beta1constants.OperationRotateRolloutWorkers)) {
	...
	for _, mOp := range maintenanceOperations {
		if !availableShootMaintenanceOperations.Has(mOp) && !strings.HasPrefix(mOp, v1beta1constants.OperationRotateRolloutWorkers) {
			allErrs = append(allErrs, field.NotSupported(fldPathMaintOp, mOp, sets.List(availableShootMaintenanceOperations)))
		} else if len(maintenanceOperations) > 1 && !availableShootOperationsToRunInParallel.Has(mOp) && !strings.HasPrefix(mOp, v1beta1constants.OperationRotateRolloutWorkers) {
		...
		if helper.IsShootInHibernation(shoot) &&
			(forbiddenShootOperationsWhenHibernated.Has(mOp) || strings.HasPrefix(mOp, v1beta1constants.OperationRotateRolloutWorkers)) {
	...
	for _, mOp := range maintenanceOperations {
		...
		case v1beta1constants.OperationRotateCredentialsStart, v1beta1constants.OperationRotateCredentialsStartWithoutWorkersRollout:
			for _, op := range operations {
				if sets.New(v1beta1constants.OperationRotateCAStart, v1beta1constants.OperationRotateCAStartWithoutWorkersRollout, v1beta1constants.OperationRotateServiceAccountKeyStart, v1beta1constants.OperationRotateServiceAccountKeyStartWithoutWorkersRollout, v1beta1constants.OperationRotateETCDEncryptionKeyStart).Has(op) {
	...
	for _, op := range operations {
		...
		case v1beta1constants.OperationRotateCredentialsStart, v1beta1constants.OperationRotateCredentialsStartWithoutWorkersRollout:
			for _, mOp := range maintenanceOperations {
				if sets.New(v1beta1constants.OperationRotateCAStart, v1beta1constants.OperationRotateCAStartWithoutWorkersRollout, v1beta1constants.OperationRotateServiceAccountKeyStart, v1beta1constants.OperationRotateServiceAccountKeyStartWithoutWorkersRollout, v1beta1constants.OperationRotateETCDEncryptionKeyStart).Has(mOp) {
					allErrs = append(allErrs, field.Forbidden(fldPathOp, fmt.Sprintf("operation '%s' is not permitted when maintenance operation is '%s'", op, mOp)))
				}
			}
		case v1beta1constants.OperationRotateCredentialsComplete:

...
var _ = Describe("Builder", func() {
	const (
		conditionType = gardencorev1alpha1.ConditionType("Test")
		// re-decalared so the underlying constant is not changed
		unknowStatus       = gardencorev1alpha1.ConditionStatus("Unknown")
		fooStatus          = gardencorev1alpha1.ConditionStatus("Foo")
		bazReason          = "Baz"
		fubarMessage       = "FuBar"
	...
	var (
		...
		codes           = []gardencorev1alpha1.ErrorCode{
			gardencorev1alpha1.ErrorInfraDependencies,
		}
	)
	...
	Describe("#Build", func() {
		var (
			result  gardencorev1alpha1.Condition
			updated bool
		...
		Context("Full override", func() {
			...
			It("should return correct result", func() {
				Expect(result).To(Equal(gardencorev1alpha1.Condition{
					Type:               conditionType,
					Status:             gardencorev1alpha1.ConditionStatus("SomeNewStatus"),
					LastTransitionTime: defaultTime,
					LastUpdateTime:     defaultTime,

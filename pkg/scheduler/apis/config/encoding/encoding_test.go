...
var _ = Describe("Encoding", func() {
	var (
		config = &schedulerconfigv1alpha1.SchedulerConfiguration{
	...
	Describe("#schedulerConfiguration", func() {
		It("should decode the raw config to a SchedulerConfiguration without defaults", func() {
			result, err := DecodeSchedulerConfiguration(&runtime.RawExtension{Raw: encode(config)}, false)

			Expect(err).NotTo(HaveOccurred())
			Expect(result).To(Equal(config))
		})

		It("should decode the raw config to a SchedulerConfiguration with defaults", func() {
			configWithDefaults := config.DeepCopy()
			schedulerconfigv1alpha1.SetObjectDefaults_SchedulerConfiguration(configWithDefaults)

			result, err := DecodeSchedulerConfiguration(&runtime.RawExtension{Raw: encode(config)}, true)
		...
		It("should return the raw config object if it's already set", func() {
			result, err := DecodeSchedulerConfiguration(&runtime.RawExtension{Object: config}, true)
	...
	Describe("#DecodeSchedulerConfigurationFromBytes", func() {
		It("should decode the byte slice into a SchedulerConfiguration", func() {
			result, err := DecodeSchedulerConfigurationFromBytes(encode(config), false)

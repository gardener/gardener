...
var _ = Describe("NamespacedCloudProfile Reconciler", func() {
	...
	Context("merge machine images", func() {
		...
		It("should ignore versions specified only in NamespacedCloudProfile", func() {
			...
			c.EXPECT().Patch(gomock.Any(), gomock.AssignableToTypeOf(&gardencorev1beta1.NamespacedCloudProfile{}), gomock.Any()).DoAndReturn(func(_ context.Context, o client.Object, patch client.Patch, _ ...client.PatchOption) error {
				Expect(patch.Data(o)).To(BeEquivalentTo(`{"status":{"cloudProfileSpec":{"machineImages":[{"name":"test-image","updateStrategy":"major","versions":[{"architectures":["amd64"],"cri":[{"name":"containerd"}],"kubeletVersionConstraint":"==1.30.0","version":"1.0.0"}]}]}}}`))
				return nil
			})
		...
		It("should merge MachineImages correctly", func() {
			...
			c.EXPECT().Patch(gomock.Any(), gomock.AssignableToTypeOf(&gardencorev1beta1.NamespacedCloudProfile{}), gomock.Any()).DoAndReturn(func(_ context.Context, o client.Object, patch client.Patch, _ ...client.PatchOption) error {
				Expect(patch.Data(o)).To(BeEquivalentTo(fmt.Sprintf(`{"status":{"cloudProfileSpec":{"machineImages":[{"name":"test-image","updateStrategy":"major","versions":[{"architectures":["amd64"],"cri":[{"name":"containerd"}],"expirationDate":"%s","kubeletVersionConstraint":"==1.30.0","version":"1.0.0"}]}]}}}`, newExpiryDate.UTC().Format(time.RFC3339)))) //
				return nil
			})
	...
	Context("merge machine types", func() {
		...
		It("should successfully add types specified only in NamespacedCloudProfile", func() {
			...
			c.EXPECT().Patch(gomock.Any(), gomock.AssignableToTypeOf(&gardencorev1beta1.NamespacedCloudProfile{}), gomock.Any()).DoAndReturn(func(_ context.Context, o client.Object, patch client.Patch, _ ...client.PatchOption) error {
				Expect(patch.Data(o)).To(BeEquivalentTo(`{"status":{"cloudProfileSpec":{"machineTypes":[{"cpu":"1","gpu":"5","memory":"3Gi","name":"test-type-namespaced"}]}}}`))
				return nil
			})
	...
	Context("merge regions", func() {
		BeforeEach(func() {
			cloudProfile.Spec.Regions = []gardencorev1beta1.Region{{Name: "test-region"}}
		})

		It("should successfully add regions specified only in NamespacedCloudProfile", func() {
			namespacedCloudProfile.Spec.Regions = []gardencorev1beta1.Region{{Name: "test-region-namespaced"}}
			...
			c.EXPECT().Patch(gomock.Any(), gomock.AssignableToTypeOf(&gardencorev1beta1.NamespacedCloudProfile{}), gomock.Any()).DoAndReturn(func(_ context.Context, o client.Object, patch client.Patch, _ ...client.PatchOption) error {
				Expect(patch.Data(o)).To(BeEquivalentTo(`{"status":{"cloudProfileSpec":{"regions":[{"name":"test-region"},{"name":"test-region-namespaced"}]}}}`))
				return nil
			})

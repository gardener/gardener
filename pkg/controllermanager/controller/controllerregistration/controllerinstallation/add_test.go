...
var _ = Describe("Add", func() {
	Describe("BackupBucketPredicate", func() {
		...
		Context("for seeds", func() {
			BeforeEach(func() {
				p = BackupBucketPredicate(SeedKind)
			})
		...
		Context("for shoots", func() {
			BeforeEach(func() {
				p = BackupBucketPredicate(ShootKind)
			})
			...
			Describe("#Create", func() {
				...
				It("should return true when seed name is not set", func() {
					Expect(p.Create(event.CreateEvent{Object: backupBucket})).To(BeTrue())
				})
			})
	...
	Describe("BackupEntryPredicate", func() {
		...
		Context("for seeds", func() {
			BeforeEach(func() {
				p = BackupEntryPredicate(SeedKind)
			})
		...
		Context("for shoots", func() {
			BeforeEach(func() {
				p = BackupEntryPredicate(ShootKind)
			})
	...
	Describe("ShootPredicate", func() {
		...
		Context("for seeds", func() {
			...
			Describe("#Update", func() {
				...
				It("should return false even though deletion timestamp was set changed", func() {
					oldShoot := shoot.DeepCopy()
		...
		Context("for shoots", func() {
			BeforeEach(func() {
				p = ShootPredicate(ShootKind)
			})

			Describe("#Create", func() {
				It("should return false when shoot is not self-hosted", func() {
					Expect(p.Create(event.CreateEvent{Object: shoot})).To(BeFalse())
				})

				It("should return true when shoot is self-hosted", func() {
			...
			Describe("#Update", func() {
				...
				It("should return false because there a relevant change but shoot is not self-hosted", func() {
					oldShoot := shoot.DeepCopy()
					shoot.Spec.Provider.Workers = []gardencorev1beta1.Worker{{}}
					Expect(p.Update(event.UpdateEvent{ObjectNew: shoot, ObjectOld: oldShoot})).To(BeFalse())
				})
				...
				It("should return true because deletion timestamp changed", func() {
					shoot.Spec.Provider.Workers = append(shoot.Spec.Provider.Workers, gardencorev1beta1.Worker{ControlPlane: &gardencorev1beta1.WorkerControlPlane{}})
					oldShoot := shoot.DeepCopy()
					shoot.DeletionTimestamp = &metav1.Time{}
					Expect(p.Update(event.UpdateEvent{ObjectNew: shoot, ObjectOld: oldShoot})).To(BeTrue())
	...
	Describe("ControllerInstallationPredicate", func() {
		...
		Context("for seeds", func() {
			BeforeEach(func() {
				p = ControllerInstallationPredicate(SeedKind)
			})
		...
		Context("for shoots", func() {
			BeforeEach(func() {
				p = ControllerInstallationPredicate(ShootKind)
			})

			Describe("#Create", func() {
				It("should return false when shootref is not set", func() {
					Expect(p.Create(event.CreateEvent{})).To(BeFalse())
				})

				It("should return true when shootref is set", func() {
					controllerInstallation.Spec.ShootRef = &corev1.ObjectReference{}
					Expect(p.Create(event.CreateEvent{Object: controllerInstallation})).To(BeTrue())
				})
			})
			...
			Describe("#Update", func() {
				...
				It("should return false because seedref is set even though Required condition got added", func() {
					oldControllerInstallation := controllerInstallation.DeepCopy()

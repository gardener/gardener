...
var _ = Describe("Shoot Maintenance", func() {
	...
	Describe("#computeCredentialsRotationResults", func() {
		...
		BeforeEach(func() {
			shoot = &gardencorev1beta1.Shoot{
				...
				Spec: gardencorev1beta1.ShootSpec{
					Maintenance: &gardencorev1beta1.Maintenance{
						AutoRotate: &gardencorev1beta1.MaintenanceAutoRotate{
							ETCDEncryptionKey:        ptr.To(true),
							SSHKeypairForWorkerNodes: ptr.To(true),
							ObservabilityPasswords:   ptr.To(true),
		...
		It("should return empty results if no credentials rotation is required", func() {
			shoot.Spec.Maintenance.AutoRotate.ETCDEncryptionKey = ptr.To(false)
			shoot.Spec.Maintenance.AutoRotate.SSHKeypairForWorkerNodes = ptr.To(false)
			shoot.Spec.Maintenance.AutoRotate.ObservabilityPasswords = ptr.To(false)
			...
			shoot.Status.Credentials = &gardencorev1beta1.ShootCredentials{
				Rotation: &gardencorev1beta1.ShootCredentialsRotation{
					ETCDEncryptionKey: &gardencorev1beta1.ETCDEncryptionKeyRotation{
						LastCompletionTime: &metav1.Time{Time: now.Add(-24*time.Hour - 1*time.Second)},
		...
		It("should not return results when the rotation period has not passed", func() {
			shoot.Status.Credentials = &gardencorev1beta1.ShootCredentials{
				Rotation: &gardencorev1beta1.ShootCredentialsRotation{
					ETCDEncryptionKey: &gardencorev1beta1.ETCDEncryptionKeyRotation{
						LastCompletionTime: &metav1.Time{Time: now.Add(-24 * time.Hour)},
					},
					SSHKeypair: &gardencorev1beta1.ShootSSHKeypairRotation{
		...
		DescribeTable("etcd encryption key rotation phase", func(phase gardencorev1beta1.CredentialsRotationPhase, shouldSucceed bool) {
			...
			Entry("should return successful result for completed phase", gardencorev1beta1.RotationCompleted, true),
			Entry("should return unsuccessful result for preparing phase", gardencorev1beta1.RotationPreparing, false),
			Entry("should return unsuccessful result for prepared phase", gardencorev1beta1.RotationPrepared, false),
			Entry("should return unsuccessful result for completing phase", gardencorev1beta1.RotationCompleting, false),
		)
	})
	...
	Describe("#getOperation", func() {
		...
		DescribeTable("#getOperation",
			...
			Entry("should return rotate-etcd-encryption-key operation when it is not part of the result updates", ptr.To("rotate-etcd-encryption-key"),
			...
			Entry("should return reconcile operation when maintenance operation is rotate-etcd-encryption-key and it is part of results", ptr.To("rotate-etcd-encryption-key"),
				map[string]updateResult{
					"etcd-encryption-key": {},
				}, "reconcile"),

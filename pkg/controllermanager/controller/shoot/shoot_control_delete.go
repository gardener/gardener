...
func (c *Controller) runDeleteShootFlow(o *operation.Operation) *gardencorev1alpha1.LastError {
	...
	var (
		...
		// Redeploy the control plane, worker extensions, and kube-controller-manager to make sure all components that depend on the
		...
		computeShootOSConfig = g.Add(flow.Task{
			...
			Fn:           flow.SimpleTaskFn(hybridBotanist.ComputeShootOperatingSystemConfig).RetryUntilTimeout(defaultInterval, defaultTimeout).DoIf(cleanupShootResources && workerDeploymentNeeded && !shootNamespaceInDeletion),
			Dependencies: flow.NewTaskIDs(deploySecrets, initializeShootClients),
		})

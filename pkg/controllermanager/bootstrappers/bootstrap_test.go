...
var _ = Describe("#bootstrapCluster", func() {
	...
	BeforeEach(func() {
		...
		fakeDiscoveryClient.FakedServerVersion = &version.Info{GitVersion: "1.30.4"}
	...
	It("should return an error because the garden version is too low", func() {
		fakeDiscoveryClient.FakedServerVersion.GitVersion = "1.29.5"
		Expect(bootstrapCluster(ctx, fakeClient, fakeDiscoveryClient, sm)).To(MatchError(ContainSubstring("the Kubernetes version of the Garden cluster must be at least 1.30")))
	})

...
var _ = Describe("#bootstrapCluster", func() {
	...
	It("should return an error because the garden version is too low", func() {
		...
		Expect(bootstrapCluster(ctx, fakeClient, fakeDiscoveryClient, sm)).To(MatchError(ContainSubstring("the Kubernetes version of the Garden cluster must be at least 1.32")))
	})

...
In the unmanaged-infrastructure/gind scenario, if a container named
'gind-machine-0' is found via 'docker inspect', a fresh admin client
certificate is generated by 'docker exec'ing into that container.
Otherwise, the kubeconfig is fetched from the Secret
'shoot--<namespace>--<name>/kubeconfig' on the bootstrap cluster
pointed to by --runtime-kubeconfig.

...
function run() {
  if ! "$repo_root"/hack/check-skaffold-deps-for-binary.sh "$operation" --skaffold-file "dev-setup/$1" --binary "$2" --skaffold-config "$3"; then
    success=false
  fi
}
...
# skaffold-cloud-provider-local.yaml
run "skaffold-cloud-provider-local.yaml" "cloud-controller-manager-local" "cloud-provider-local"

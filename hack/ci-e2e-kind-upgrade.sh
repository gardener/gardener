...
function copy_kubeconfig_files_to_old_gardener_version_folder() {
  ...
  # Copy the registry CA cert generated by infra.sh. infra.sh only runs in the 'next version' folder, so this file is
  # absent here. The skaffold build hook (generate-patch-gardenlet.sh) reads it to inject caBundle.inline into the
  # gardenlet imageVectorOverwrite.
  local new_dev_setup="${KUBECONFIG_RUNTIME_CLUSTER%%/dev-setup/*}/dev-setup"
  local registry_ca_crt="infra/registry/tls/ca.crt"
  if [[ -f "$new_dev_setup/$registry_ca_crt" ]]; then
    mkdir -p "dev-setup/$(dirname "$registry_ca_crt")"
    cp "$new_dev_setup/$registry_ca_crt" "dev-setup/$registry_ca_crt"
  fi

  # Copy registry CA secrets generated by infra.sh.
  for registry_ca_path in \
    "gardenlet/base/secret-registry-ca.yaml" \
    "extensions/provider-local/components/extension/secret-registry-ca.yaml"; do
    [[ -f "$new_dev_setup/$registry_ca_path" ]] && [[ -d "dev-setup/$(dirname "$registry_ca_path")" ]] && \
      cp "$new_dev_setup/$registry_ca_path" "dev-setup/$registry_ca_path"
  done
}

...
is_custom_artifact="$(echo "$skaffold_yaml" | yq eval "select(.metadata.name == \"$skaffold_config_name\") | .build.artifacts[] | select(.image == \"$binary_name\") | has(\"custom\")" -)"
...
echo "$skaffold_yaml" |\
  yq eval "select(.metadata.name == \"$skaffold_config_name\") | .build.artifacts[] | select(.image == \"$binary_name\") | $yq_dependencies_selector.paths[]?" - |\
  sort -f |\
...
case "$operation" in
  ...
  update)
    ...
    yq eval -i "select(.metadata.name == \"$skaffold_config_name\") |= .build.artifacts[] |= select(.image == \"$binary_name\") |= $yq_dependencies_selector.paths |= [$actual_dependencies]" "$skaffold_file"

...
| BootstrapTokenProvidedByWorker | `false` | `Alpha` | `1.22.0` | |
...
* `BootstrapTokenProvidedByWorker` enables a security enhancement for the bootstrap flow of the kubelet. The bootstrap-token is now uniquely generated for each VM and invalidated after successful creation. This requires support in the worker controller and in the os-extension (i.e. gardener-extension-os-gardenlinux >`v.0.9.0`)

...
| DoNotCopyBackupCredentials               | `gardenlet`                        | Disables the copying of Shoot credentials as backup credentials when the Shoot is used as a ManagedSeed. Operators are responsible for providing the credentials for backup explicitly. Credentials that were already copied will be labeled with `gardener.cloud/secret-status=previously-managed` and would have to be cleaned up by operators.                                                                                                                                                                                                        |

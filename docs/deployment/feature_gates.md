...
* `DisableDNSProviderManagement` turns off the creation, update, and deletion of `DNSProviders` as specified in the shoot manifest in section `spec.dns.providers`. This feature is only effective if the feature `UseDNSRecords` is `true`. The purpose of this feature is to move management of `DNSProviders` to the `shoot-dns-service` extension.

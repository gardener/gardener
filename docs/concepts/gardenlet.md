...
- In case `GardenletConfiguration.controllers.shoot.reconcileInMaintenance` is enabled (disabled by default), gardenlet performs regular shoot reconciliations only once in the respective maintenance time window (`GardenletConfiguration.controllers.shoot.reconcileInMaintenance.syncPeriod` is ignored). Gardenlet randomly distributes shoot reconciliations over the maintenance time window to avoid high bursts of reconciliations.
- In case `Shoot.spec.maintenance.confineSpecUpdateRollouts` is enabled (disabled by default), changes to the shoot specification are not rolled out immediately but only during the respective maintenance time window (see [this doc](../usage/shoot_maintenance.md)).

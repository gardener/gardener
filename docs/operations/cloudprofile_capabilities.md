...
2. **Value order** (most to least preferred): Within each capability, the first value is the most-preferred. For example, `storageAccess: [NVMe, SCSI]` means "prefer NVMe; use SCSI only if necessary."
...
See also: [Shoot Maintenance — Automatic Version Updates](../usage/shoot/shoot_maintenance.md#automatic-version-updates).
...
- **Shoot validator admission** — rejects user-selected machine type / image combinations that are incompatible.
  See [`ShootValidator` admission plugin](../concepts/apiserver-admission-plugins.md#shootvalidator); implemented in [`plugin/pkg/shoot/validator/admission.go`](../../plugin/pkg/shoot/validator/admission.go) (function `validateMachineCapabilities`).
- **Shoot mutator admission** — selects a default image when the user does not specify one.
  See [`ShootMutator` admission plugin](../concepts/apiserver-admission-plugins.md#shootmutator); implemented in [`plugin/pkg/shoot/mutator/admission.go`](../../plugin/pkg/shoot/mutator/admission.go).
- **Maintenance controller** — picks the upgrade target during automatic image maintenance.
  See [Shoot Maintenance — Automatic Version Updates](../usage/shoot/shoot_maintenance.md#automatic-version-updates); implemented in [`pkg/controllermanager/controller/shoot/maintenance/helper/helper.go`](../../pkg/controllermanager/controller/shoot/maintenance/helper/helper.go) (function `FilterMachineImageVersions`).
- **Provider extension worker controller** — selects the actual provider image reference for a given flavor.
  Lives in each provider-extension repo (e.g. `gardener-extension-provider-aws`); the reusable helper for extensions is [`extensions/pkg/controller/worker/cloudprofile.go`](../../extensions/pkg/controller/worker/cloudprofile.go) (`FindBestImageFlavor`). An in-tree reference implementation is the local provider's [`pkg/provider-local/controller/worker/machine_images.go`](../../pkg/provider-local/controller/worker/machine_images.go).
...
2. **Order from best to legacy.** This ensures that maintenance auto-upgrades steer fleets toward the best possible image when multiple flavors are valid. Example: `storageAccess: [NVMe, SCSI]` prefers `NVMe`.
...
2. **Provider extensions** are upgraded to a version that understands `capabilityFlavors` in their `providerConfig` section. `CloudProfile`s still work without capabilities.

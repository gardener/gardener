...
Additionally, Gardener labels `OperatingSystemConfig` resources generated for worker pools with the same container runtime labels (`containerruntime.worker.gardener.cloud/<container-runtime-type-value>=true`) so that extension mutating webhooks can target only relevant `OperatingSystemConfig` objects using an `ObjectSelector`.

...
- `InPlaceOrRecreate`: In this mode, recommendations are computed and applied to `Pod`(s) using the [in-place resource updates](https://kubernetes.io/blog/2025/05/16/kubernetes-v1-33-in-place-pod-resize-beta/) mechanism. In case of failure, it falls back to _recreation_ and _evicts_ the targeted `Pod`.

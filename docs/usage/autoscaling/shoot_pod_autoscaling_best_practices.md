...
- As a rule of thumb, do not set initial requests less than VPA's `minAllowed` or vpa-recommender's global minimum recommendation (`5m` and `10M`). In Gardener, vpa-recommender is configured to run with `--pod-recommendation-min-cpu-millicores=5` and `--pod-recommendation-min-memory-mb=10` ([ref](https://github.com/gardener/gardener/blob/957e4c7/pkg/component/autoscaling/vpa/recommender.go#L205-L206)). For the reasoning, see [VPA Recommender Options](#vpa-recommender-options). If the Pod has a container with resource requests less than the VPA's `minAllowed` or `5m` and `10M`, the Pod will be evicted immediately. The reason is that the VPA's lower bound recommendation is being set to the `minAllowed` or `5m` and `10M`. vpa-updater evicts immediately if the container's resource requests are less than the VPA's lower bound.

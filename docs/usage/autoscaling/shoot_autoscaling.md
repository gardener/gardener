...
There are [general options for `cluster-autoscaler`](../../api-reference/core.md#clusterautoscaler), and these values will be used for all worker groups except for those overwriting them. Additionally, there are some [`cluster-autoscaler` flags to be set per worker pool](../../api-reference/core.md#clusterautoscaleroptions). They override any general value such as those specified in the general flags above.
...
The `Shoot` API allows to configure most of the [flags of the `horizontal-pod-autoscaler`](../../api-reference/core.md#horizontalpodautoscalerconfig).
...
The `Shoot` API allows to configure a few [flags of the `vertical-pod-autoscaler`](../../api-reference/core.md#verticalpodautoscaler).

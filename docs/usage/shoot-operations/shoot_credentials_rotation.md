...
|---|---|---|---|
| **Infrastructure credentials** | Project-scoped — a single `Secret` can be shared across multiple Shoots | Fully user-managed | Update the `Secret`, wait for Shoot reconciliation |
...
Infrastructure credentials are **not** part of the Shoot itself. They are `Secret`s stored in the garden cluster's project namespace and referenced by Shoots via a `CredentialsBinding`. In contrast, Shoot credentials (CAs, SSH keys, ETCD encryption keys, etc.) are generated per Shoot by Gardener and rotate through well-defined phases driven by `kubectl annotate` operations on the Shoot resource.

## Infrastructure Credentials (Project-Scoped)

> This section covers rotation of cloud provider credentials you supplied to Gardener — for example, an Azure Service Principal, AWS access keys, a GCP service account key, or OpenStack credentials. These are project-level `Secret`s referenced by Shoots via `CredentialsBinding`; they are not part of the Shoot itself and may be shared across multiple Shoots.

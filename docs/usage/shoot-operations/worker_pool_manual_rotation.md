...
There may be cases when an end used might want to trigger a manual worker pool rollout. For example, the dual-stack migration requires to roll nodes.
...
```bash
kubectl -n <shoot-namespace> annotate shoot <shoot-name> gardener.clloud/operation=rollout-workers=<pool1-name>[,<pool2-name>,...]
```

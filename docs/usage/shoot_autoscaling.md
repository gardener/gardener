...
The following flags are general options for `cluster-autoscaler`, and these values will be used for all worker groups except for those who have specific values defined:
...
The following flags allow some `cluster-autoscaler` flags to be set per worker group. They are specified along with the worker and override any general value such as those specified in the flags above

...
- Scaling horizontally doesn't cause downtimes (at least not when out-scaling and only one replica is affected when in-scaling), but scaling vertically does (if the pod runs OOM anyway, but also when new recommendations are applied, resource requests for existing pods may be changed, which causes the pods to be rescheduled). The discussions around the in-place VPA update-policy (see [KEP 1287](https://github.com/kubernetes/enhancements/blob/master/keps/sig-node/1287-in-place-update-pod-resources/README.md)) took a very long time, but was being graduated to beta and enabled by default since Kubernetes v1.33. You can find the implementation in Kubernetes [here](https://github.com/kubernetes/kubernetes/pull/102884) and the implementation in VPA [here](https://github.com/kubernetes/autoscaler/issues/4016).

...
```bash
ip addr add 127.0.0.2 dev lo0                                     # adding 127.0.0.2 ip to the loopback interface
```
...
This command sets up a new KinD cluster named `gardener-local2` and stores its kubeconfig in the `./example/gardener-local/kind/kubeconfig2` file. You will need this file when starting the `provider-local` extension controller for the second seed cluster.
...
```bash
export KUBECONFIG=./example/gardener-local/kind/kubeconfig2       # setting KUBECONFIG to point to second kind cluster

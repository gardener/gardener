...
- [`metav1validation.ValidateLabelName`](https://pkg.go.dev/k8s.io/apimachinery/pkg/apis/meta/v1/validation#ValidateLabelName)
...
- When working with a `ValidatingWebhookConfiguration`s, strictly follow the [Admission Webhook Good Practices](https://kubernetes.io/docs/concepts/cluster-administration/admission-webhooks-good-practices/).
- When introducing a new field, consider if it should be immutable or not.
  - Consider if updates to the field value should be allowed and are supported by the underlying controller. If not, consider making the field immutable. Add a doc string to the field to denote the field is immutable.

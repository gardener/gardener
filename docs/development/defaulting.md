...
- If there is only one type under an api group then we can just have `types.go` and a corresponding `defaults.go`. For instance, `resourcemanager` api has only one [`types.go`](../../pkg/apis/config/resourcemanager/v1alpha1/types.go), hence in this case only [`defaults.go`](../../pkg/apis/config/resourcemanager/v1alpha1/defaults.go) file would suffice.

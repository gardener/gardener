...
- Maintain the Kubernetes `kube-controller-manager` controllers for each API group used in deploying required KCM controllers based on active APIs:
  ...
  - Make sure that the API groups in [this](https://github.com/gardener/gardener/blob/master/kg/utils/validation/apigroups/apigroups.go) file are in sync with the groups in [this](https://github.com/gardener/gardener/blob/master/pkg/utils/kubernetes/controllers.go) file. For eg, `core/v1` is replaced by the script as `v1` and `apiserverinternal` as `internal`. This is because the API groups registered by the apiserver and the api path imported by the controllers might be slightly different in some cases.

...
- Maintain the Kubernetes `kube-controller-manager` controllers for each API group used in deploying required KCM controllers based on active APIs:
  ...
  - Maintain the names of controllers used for workerless Shoots, [here](https://github.com/gardener/gardener/blob/6988da80bae6ba827d63535655f28885d91b0a23/pkg/component/kubernetes/controllermanager/controllermanager.go#L744-L766) after carefully evaluating whether they are needed if there are no workers. It is quite possible google or the kubernetes documentation to be insufficient in finding out what a given controller does. Therefore searching the `kubernetes/kubernetes` repo for the controller name and through that tracking the PR that introduces the controller might be nessesary.

...
3. Extending the expiration date of Kubernetes versions. Teams might need to hang on to an older version of Kubernetes as they might not be ready for an upgrade as of the set expiration date. Given that the cloud profile is a cluster scoped resource, it is not currently not possible to only extend the expiration date for shoots in one project but only centrally for all projects.
...
Cloud Profiles are non namespaces resources. This means that in a typical gardener installation, only a handful of cloud profiles (i.e. one per cloud provider) exist. They can be consumed by any project's clusters. Consequently, when a project requires changes to a cloud profiles for any of the reasons mentioned in the [context section](#context), they are changed for the entire gardener landscape. Since some projects might require frequent changes to a cloud profile, this become quite a cumbersome process. Additionally it adds confusion to every projects workflow if say for example a team requires an extension of the expiration date of a kubernetes version since they will be wondering why it was extended.
...
- Add different, possibly unsupported Kubernetes versions or machine images versions and names
...
First of all, a new, namespaced API object is defined the `PrivateCloudProfile`, it's type definition is very similar to the `CloudProfile` object but it omits the `type`, `providerConfig` and `seedSelector` fields. The `type` field is omitted as it is inherited from the parent cloud profile. The `providerConfig` field is omitted as gardener itself does not know anything about the structure of this field and cannot merge it accordingly. The `seedSelector` field is omitted as it is to complex for this GEP but may be added in the future.
...
Currently, a shoots reference to cloud profile is immutable. When enabling private cloud profiles, it is necessary to allow a change to the cloud profile reference. Although it is very important to only allow a change to a private cloud profile that uses the shoots current cloud profile as a parent. Additionally, the change may go in the other direction, changing from a private cloud profile the private cloud profiles parent cloud  profile.
...
and a private cloud profile the private cloud profile from the [manifest section](#manifest).
...
The rendering is done by a new custom controller registered to the `gardener-controller-manager`. It a merge conflict is to appear during the rendering process, it should be noted somewhere in the status and the parent cloud profile should be preferred.

### Custom rbac verb

To prevent users from entering arbitrary values in the `kubernetes` and `machineImages` fields, two custom rbac verbs may be introduced. It cloud then be checked if the user that is creating or updating the `kubernetes` or `machineImages` field is authorized to do so.
...
Expired Kubernetes versions in cloud profiles are currently being removed manually. This is not an issue since there is a very limited amount of cloud profiles. However, it any project administrator is allowed to create private cloud profile which can then have its Kubernetes versions expiration dates extended, it cloud become a very cumbersome process to manually remove all the expired Kubernetes versions. It is therefore proposed to implement a custom controller managed my `gardener-controller-manager`, that reconciles private cloud profiles (and possibly even regular cloud profiles) and removes any Kubernetes versions that are past their expiration date. The same could be done for machine images versions.

However, a problem arises as the kubernetes/machine image versions are referenced in the `providerConfig` and would therefore need to be removed manually anyways.

### Cross project sharing

A use case could be defined where a private cloud profile might want to shared across multiple projects and not just be used within the project it was created id. However this use case seems to be very slim so this functionality will not be implemented as of now. Although, when taking a broader view with Private Seeds and Cloud in Country, this feature is going to be necessary at some point. Still, is is not planned to be implemented as of this GEP.
...
Instead of specifying a private cloud profile resource, an end user could be allowed to enter arbitrary names in fields such as `machineTypes`. The entry would then, if the user enters a wrong value, throw an error when provisioning the resource at the cloud provider. However, this approach does not seam feasible as the metadata that is specified in a cloud profile like amount of cpus and ram is used in the trail clusters to validate quotas. Therefore, an exception would need to be developed for this specific, and possible other, use cases.

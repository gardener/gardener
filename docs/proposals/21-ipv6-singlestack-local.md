...
Container images from `docker.io` used on shoots, seeds and extensions are hard-coded to `mirror.gcr.io` in all respective `images.yaml`. The Google container registry supports dual-stack access and has no rate-limit.

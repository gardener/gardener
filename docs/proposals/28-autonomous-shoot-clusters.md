...
Augmenting Gardener's capability to also manage the initial cluster will help to reduce operational complexity and to enable new use-cases and scenarios.
...
This way, they can be easier translated into static pods and distributed to the respective control plane nodes.
...
> The details are still subject of discussion and will become clearer in the future.
> Since we need a "bootstrap `etcd`" anyways to get the cluster up and running, we can proceed with this GEP and its implementation in parallel while the discussion around how `etcd-druid` can take over the management is ongoing.
...
For convenience, the `gardenadm token create` command can also automatically print the [`gardenadm join`](#gardenadm-join) command for the respective token.
This way, the user can just copy it and join the desired machine without tediously defining the command manually.

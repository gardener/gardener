...
the `kube-rbac-proxy`[[3]] application. It serves HTTPS and authenticates and authorizes each request against the runtime
cluster via the TokenReview[[4]] and SubjectAccessReview[[5]] K8s APIs, before forwarding it to the internal, insecure
loopback port.
...
### Alternatives
A detailed review of existing PVC autoscalers was performed as part of developing `pvc-autoscaler`. Its findings are
discussed in detail in the `pvc-autoscaler` evaluation of existing autoscalers [[2]].
The present document accounts for those findings, and also for recent developments.
...
- [[2]] `pvc-autoscaler` evaluation of existing autoscalers
- [[3]] `kube-rbac-proxy` GitHub project
- [[4]] Kubernetes TokenReview API
- [[5]] Kubernetes SubjectAccessReview API

[1]: https://github.com/gardener/pvc-autoscaler
[2]: https://github.tools.sap/I746790/notes/tree/master/pvc-autoscaler#evaluate-existing-pvc-autoscalers
[3]: https://github.com/brancz/kube-rbac-proxy
[4]: https://kubernetes.io/docs/reference/kubernetes-api/authentication-resources/token-review-v1/
[5]: https://kubernetes.io/docs/reference/kubernetes-api/authorization-resources/subject-access-review-v1/

	...
	BeforeEach(func() {
		...
		operatingSystemConfig = &extensionsv1alpha1.OperatingSystemConfig{
			Spec: extensionsv1alpha1.OperatingSystemConfigSpec{
				...
				Units: []extensionsv1alpha1.Unit{unit1, unit2},
			},
...
func assertFileOnDisk(fakeFS afero.Fs, path, expectedContent string, fileMode uint32) {
	description := "file path " + path

	content, err := afero.ReadFile(fakeFS, path)
...
func assertNoFileOnDisk(fakeFS afero.Fs, path string) {
	_, err := afero.ReadFile(fakeFS, path)

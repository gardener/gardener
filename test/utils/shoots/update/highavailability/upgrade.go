...
func verifyEnvoyFilterInIstioNamespace(ctx context.Context, seedClient kubernetes.Interface, shootName string, checkLabels bool) {
	...
	CEventually(ctx, func(g Gomega) {
		...
		// TODO(Wieneo): Remove this when feature gate RemoveAPIServerProxyLegacyPort is removed
		...
		// - Feature Gate "IstioTLSTermination" enabled, "RemoveAPIServerProxyLegacyPort" enabled = 1 envoy filter
		...
		istioIngressGateway := &corev1.Service{}
		if err := seedClient.Client().Get(ctx, client.ObjectKey{Name: v1beta1constants.DefaultSNIIngressServiceName, Namespace: v1beta1constants.DefaultSNIIngressNamespace}, istioIngressGateway); err != nil {
			g.Expect(err).To(Not(HaveOccurred()))
		}

		for _, k := range istioIngressGateway.Spec.Ports {
			if k.Port == 8443 {
		...
		if istioTlsTerminationEnabled && !removeAPIServerProxyLegacyPortEnabled {
			expectedEnvoyFilters = 2
		}

		if (!istioTlsTerminationEnabled && !removeAPIServerProxyLegacyPortEnabled) || (istioTlsTerminationEnabled && removeAPIServerProxyLegacyPortEnabled) {
			expectedEnvoyFilters = 1
		}

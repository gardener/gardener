...
var _ = Describe("Shoot Tests", Label("Shoot", "high-availability", "single-zone"), func() {
	...
	It("Create, Upgrade (non-HA to HA with failure tolerance type 'node') and Delete Shoot", func() {
		...
		ctx, cancel = context.WithTimeout(parentCtx, 15*time.Minute)
		defer cancel()
		highavailability.UpdateAndVerify(ctx, f.ShootFramework, v1beta1.FailureToleranceTypeNode)
